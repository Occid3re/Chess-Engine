...
* Move ordering uses per-thread reusable buckets (TT, promotions, SEE-good/equal/bad captures, killers, quiets). Within each bucket we perform a small insertion sort on cached scores, then flush the buckets in priority order via `MoveContainerUtils.overwriteFromBuffer`—no `Arrays.sort` call remains in the hot path.
