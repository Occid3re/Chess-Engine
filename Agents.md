...
* The tuning loop script (`scripts/auto_tuning_loop.py`) now introspects `ParamId.java` for valid numeric ranges. It clamps candidates to those min/max values, occasionally tests sentinel values such as `0` or `-1`, and warns if `seed-tunings.yaml` is missing any declared parameter. Keep YAML keys in sync (normalised casing) to avoid silent drops.
