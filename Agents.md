...
* `BestMoveSearchTest` now mirrors the diagnostic harness from `AITest_MateThreatDiagnostics`. Each position prints an in-depth iterative-deepening trace, principal variation, and transposition-table probe summary, with aggregated JSON/TXT artifacts in `target/surefire-reports/best-move-search-*`. The refreshed defaults grant a 3 s per-position budget (`maxDepth` 24) and auto-scale multi-threading inside `SearchEnvironment.detect()`: search threads default to roughly two thirds of logical CPUs, lazy SMP helpers only spawn when there is headroom, the root-split limit hovers around `3 × cores`, and the transposition table defaults to `64 MB × cores` (capped at 512 MB). The resolved values are pushed into `System` properties so the engine proper shares the same configuration. On the current 3-core container this means two search threads, one lazy SMP worker, a root fan-out cap of nine, and a 192 MB TT, trimming the suite down to ~18 assertion failures (previously ~22) while keeping the diagnostic artifacts for the remaining outliers.
* Evaluation tuning was nudged toward centre control and piece safety. `tuning/seed-tunings.yaml` now increases central/connected pawn bonuses (18/12), slightly boosts passed pawn pressure (52) while deepening the backward pawn penalty (-16), raises pawn-threat penalties on knights/bishops (-16/-14), and sets the mid-game centre activity bonuses for knights/bishops to 3. The seed still loads automatically during `EngineTuningBootstrap`.
