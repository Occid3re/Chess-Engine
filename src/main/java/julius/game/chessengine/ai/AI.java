...
public class AI {
    ...
    public AI(Engine mainEngine) {
        ...
        this.alphaBetaSearch = new AlphaBetaSearch(this);
    ...
    boolean abortRequested(long deadline) {
    ...
    void incrementNullMoveCount() {
        nullMoveCount++;
    }

    boolean isNullMovePruningEnabled() {
    ...
    Map<Integer, Integer> getThreadLocalSeeCache() {
        return seeCacheThreadLocal.get();
    }

    TranspositionTable<TranspositionTableEntry> getTranspositionTable() {
    ...
    double alphaBeta(Engine simulatorEngine, int depth, double alpha, double beta,
                     ...
                     int extStreak) {
        return alphaBetaSearch.alphaBeta(simulatorEngine, depth, alpha, beta, isWhite,
                deadline, prevMove, plyFromRoot, extStreak);
    }
    ...
    public double evaluateBoard(Engine simulatorEngine, boolean isWhitesTurn, long deadline) {
        ...
        double score = alphaBetaSearch.quiescenceSearch(simulatorEngine, isWhitesTurn, alpha, beta, deadline, 0);
    ...
    double evaluateStaticPosition(GameState gameState, boolean isWhitesTurn, int depthOrPly) {
    ...
    synchronized boolean positionChanged() {
    ...
    void updateKillerMoves(int depth, int move) {
        threadHeuristics.get().recordKiller(depth, move);
    }

    void incrementHistory(int prevMove, int move, int depth) {
        Heuristics heuristics = threadHeuristics.get();
