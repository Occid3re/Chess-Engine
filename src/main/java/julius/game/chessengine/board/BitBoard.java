...
public class BitBoard {
    ...
    private List<SliderUpdate> prepareSliderUpdates(long impactedMask) {
        List<SliderUpdate> updates = new ArrayList<>();
        ...
        while (mask != 0) {
            ...
            if (type != null && isSliderPiece(type)) {
                ...
                long oldAttacks = computeAttacksForPiece(type, square, whitePiece);
                applyAttackDelta(whitePiece, oldAttacks, -1);
                updates.add(new SliderUpdate(square, whitePiece, type));
            }
            mask &= mask - 1;
        }
        return updates;
    ...
    private record SliderUpdate(int square, boolean whitePiece, PieceType type) {
    }
    ...
    private boolean isSquareUnderAttack(int index, boolean colorWhite) {
        ...
        if (colorWhite) {
            ...
            // Black asks: "am I attacked by White?"
            long whitePawnAttacks = ((whitePawns & ~FileMasks[0]) << 7) | ((whitePawns & ~FileMasks[7]) << 9);
            if ((whitePawnAttacks & target) != 0) return true;

            if ((KnightHelper.knightMoveTable[index] & whiteKnights) != 0) return true;

            if ((KING_ATTACKS[index] & whiteKing) != 0) return true;

            if ((bishopAttackBitmask(index) & (whiteBishops | whiteQueens)) != 0) return true;
            return (rookAttackBitmask(index) & (whiteRooks | whiteQueens)) != 0;
        }
    }
    ...
    public void performMove(int move) {
        ...
        boolean requiresFullRebuild = isCastling || isEnPassant;
        ...
        if (!requiresFullRebuild) {
            ...
            if (impactedMask != 0) {
                sliderUpdates = prepareSliderUpdates(impactedMask);
            }
        }
        ...
        if (requiresFullRebuild) {
            ...
            for (SliderUpdate update : sliderUpdates) {
                if (pieceBoard[update.square()] == update.type()) {
                    applyAttackDelta(update.whitePiece(),
                            computeAttacksForPiece(update.type(), update.square(), update.whitePiece()), 1);
                }
            }
        }
    ...
    public void undoMove(int move) {
        ...
        boolean requiresFullRebuild = isCastlingMove || isEnPassantMove;
        List<SliderUpdate> sliderUpdates = new ArrayList<>();
