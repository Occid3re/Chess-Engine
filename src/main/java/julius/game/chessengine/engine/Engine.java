...
public class Engine {
     ...
     * recycled. 8192 comfortably exceeds the product of the engine's maximum expected search depth
     * (~256 plies) and the typical number of parallel search workers (≤16), providing ample headroom
     * so concurrent threads don't observe recycled buffers while still keeping memory usage bounded.
     */
    private static final int ACTIVE_SNAPSHOT_HISTORY = 8_192;
