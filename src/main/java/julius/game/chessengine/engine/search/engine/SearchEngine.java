...
import julius.game.chessengine.ai.BestMoveDepth;
...
import julius.game.chessengine.ai.PlainFixedSizeTranspositionTable;
import julius.game.chessengine.ai.SearchDiagnostics;
import julius.game.chessengine.ai.SearchInstrumentation;
...
public class SearchEngine {
    ...
    private volatile SearchResult lastResult = SearchResult.EMPTY;
    ...
    public SearchEngine(Engine mainEngine) {
        this(mainEngine, SearchConfig.defaults());
    }

    public SearchEngine(Engine mainEngine, SearchConfig config) {
    ...
    private void iterativeDeepening(SearchTask task, Engine simulatorEngine, SplittableRandom rng) {
        ...
        for (int currentDepth = 1; currentDepth <= depthLimit; currentDepth++) {
            ...
            publishResultSnapshot(task.getBest(), currentDepth, searchContext.getCurrentPrincipalVariation());
...
private void publishResultSnapshot(BestMoveDepth best, int depth, List<MoveAndScore> pv) {
    List<MoveAndScore> variation = pv != null ? new ArrayList<>(pv) : new ArrayList<>();
    MoveAndScore first = variation.isEmpty() ? null : variation.get(0);
    int bestMove = best != null ? best.move() : (first != null ? first.getMove() : -1);
    double score = best != null ? best.score() : (first != null ? first.getScore() : 0.0);
    int effectiveDepth = Math.max(depth, best != null ? best.depth() : (first != null ? variation.size() : 0));
    long nodes = getNodesVisited();
    long elapsed = getSearchElapsedMillis();
    List<String> infoLines = buildInfoLines(variation, nodes, elapsed, effectiveDepth, score);
    lastResult = new SearchResult(bestMove, score, variation, effectiveDepth, nodes, elapsed, infoLines);
}

private List<String> buildInfoLines(List<MoveAndScore> pv, long nodes, long elapsedMillis, int depth, double score) {
    ...
    long nps = 0L;
    if (elapsedMillis > 0L) {
        long computed = (long) Math.floor((nodes * 1000.0) / elapsedMillis);
        if (computed > 0L) {
            nps = computed;
        }
    }
    ...
    if (!pv.isEmpty()) {
        String pvLine = buildPv(pv);
        if (!pvLine.isEmpty()) {
            builder.append(" pv ").append(pvLine);
        }
    }
    lines.add(builder.toString());

    if (lastDiagnostics != null) {
        int generated = lastDiagnostics.rootMovesGenerated();
        int explored = lastDiagnostics.rootMovesExplored();
        if (generated > 0 || explored > 0) {
            int difference = generated - explored;
            lines.add("info string rootmoves generated " + generated
                    + " explored " + explored
                    + " diff " + difference);
        }
    }
    ...
    if (gameState != null && gameState.getState() != null) {
        lines.add("info string gamestate " + gameState.getState());
    }
    return lines;
}
...
private String toUci(int move) {
    ...
    private void performCalculation() {
        try {
            ...
            if (bookMove != -1) {
                ...
                List<MoveAndScore> pv = List.of(new MoveAndScore(bookMove, 0.0));
                updatePrincipalVariation(pv);
                publishResultSnapshot(new BestMoveDepth(bookMove, 0.0, 1), 1, pv);
    ...
    void completeSearchTask(SearchTask task, Engine simulatorEngine) {
        ...
        if (move != -1) {
            ...
            publishResultSnapshot(best, best.depth(), searchContext.getLastCompletedPrincipalVariation());
    ...
    private double quiescenceSearch(Engine simulatorEngine, boolean isWhitesTurn,
        ...
        if (abortRequested(deadline)) return SearchEngine.EXIT_FLAG;
